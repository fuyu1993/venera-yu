...
begin
  ...
  begin
    OldVersionPath := 'C:\Program Files (x86)\漫匣';
    if DirExists(OldVersionPath) then
    begin
      DelTree(OldVersionPath, True, True, True);
      ShortcutPath := GetEnv('USERPROFILE') + '\Desktop\漫匣.lnk';
      if FileExists(ShortcutPath) then
      begin
        DeleteFile(ShortcutPath);
      end;
      ShortcutPath := 'C:\Users\Public\Desktop\漫匣.lnk';
      if FileExists(ShortcutPath) then
      begin
        DeleteFile(ShortcutPath);
      end;
...
OutputBaseFilename=漫匣-{#MyAppVersion}-windows-installer
